...
Note that both Accept and Prefer are required. Prefer specifies the response is immediate or asynchronous, which SHALL be set to respond-async. Accept specifies the format of the optional OpeartionOutcome response to the kick-off request. Currently, only application/fhir+json is supported.
