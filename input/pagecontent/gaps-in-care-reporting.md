...
As shown in Figure 2-16 and 2-17 above, gaps in care reporting leverages the quality measure evaluation capability of [$evaluate](OperationDefinition-evaluate.html) to produce one or more Individual Measure Report, which are then utilized by the gaps in care service to determine a gap status that is reported in a detected issue. Because the $care-gaps operation utilizes [$evaluate](OperationDefinition-evaluate.html), any QMIG profile requirements for [$evaluate](OperationDefinition-evaluate.html) apply to $care-gaps as well (e.g. the CQMComputableMeaure profile, depending on the server implementation).
