...
- [**Framework**](framework.html): These pages provide guidance on the set of FHIR transactions that provide a general framework to enable the exchange of measure data.
  ...
  - [**Data Exchange**](datax.html) gives guidance on the interactions between Producers and Consumers to exchange the data of interest for a measure.
...
1. <b>Submit Data</b> - Used by a Producer to submit a subset of the data of interest for a measure to a Consumer
1. <b>Collect Data</b> - Used by a Consumer to collect a subset of the data of interest for a measure from a Producer

For the exchange scenarios, the actors are Producers and Consumers:

**Producers** are the actors that hold and provide the data of interest. For the purposes of these scenarios, a _producer_ is conceptually a **Producing System**, which has access to the information generated by or derived from the delivery of healthcare — for example an EHR, a clinical data repository, or a health information exchange. Depending on the exchange, the producer may be different stakeholders such as providers or provider organizations, or any other stakeholder that holds the data of interest.

**Consumers** are the actors that request or receive the data of interest. For the purposes of these scenarios, a _consumer_ is conceptually a **Consuming System**, which may evaluate the measure itself, or aggregate the data and forward it to another system for calculation and reporting. Again, depending on the exchange, consumers may be different stakeholders, but are typically payers, registries, aggregators, health information exchanges, public health agencies, and other secondary use partners.

These roles are used in the same sense as the Data Producers and Data Consumers stakeholders in the [Quality Measurement Standards Landscape](background.html#quality-measurement-standards-landscape) diagram. Note that these are roles played in a particular exchange, not fixed categories of stakeholder: a single stakeholder may play either role, and may play different roles in different exchanges. For example, a Provider may be playing the role of Producer in a particular exchange, while the Payer may be playing the role of Consumer; in another exchange, the Payer may supply data to the Provider and so act as the Producer. See the [Glossary](glossary.html) for the full definitions of these roles.
...
In each of the scenarios described in DEQM, the FHIR API can be used directly to access the data of interest for the measure being reported. However, to support large volumes of data, [Bulk Data](https://hl7.org/fhir/uv/bulkdata/) may be used to transfer the data of interest between the clinical data repository and the reporting system. See the [Bulk Data Exchange](datax.html#bulk-data-exchange) topic for more details.

##### Aggregated Reporting

In aggregated reporting, the Reporter gathers (or receives) _data of interest_ from multiple _clinical data repositories_, allowing for reporting in a variety of ways, including multiple facilities for a single provider, as well as multi-provider organizations:
...
The DEQM framework focuses on data requests that are limited to identified measures, patients/population, measurement period, purpose, and [data of interest](glossary.html#data-of-interest). The approach is consistent with sharing data for general quality improvement and secondary use initiatives such as shared-risk contracts and value-based programs.
...
As with the exchange scenarios, these are roles played in a particular reporting interaction, not fixed categories of stakeholder. In particular, a system acting as a Consumer in an exchange scenario commonly acts as the Reporter in a subsequent reporting scenario. See the [Glossary](glossary.html) for the full definitions of these roles.
...
1. The Reporter gathers the [_data of interest_](glossary.html#data-of-interest) from available sources (potentially from a prior exchange scenario)
2. The Reporter calculates the measure, producing the MeasureReport
3. The Reporter then POSTs the MeasureReport to the Receiver
...
1. The Reporter gathers the [_data of interest_](glossary.html#data-of-interest) from available sources (potentially from a prior exchange scenario)
2. The Reporter calculates the measure, producing a MeasureReport for each attributed patient
3. The Reporter assembles a Bundle for each patient, consisting of the calculated MeasureReport and any data referenced during the calculation (i.e. evaluatedResource)
4. The Reporter POSTs the measure report Bundles to the Receiver
