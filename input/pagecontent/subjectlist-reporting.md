...
§deqm-75: When using a MeasureReport resource to represent a subject-list, the MeasureReport **SHALL** have a type-code of "subject-list" and if a subject reference is present, it **SHALL** be a reference to a Group. § §deqm-76: In addition, the resource **SHALL** include for each population a reference to a List resource that references individual level MeasureReport resources for the same measure, one for each subject in the overall population. §
