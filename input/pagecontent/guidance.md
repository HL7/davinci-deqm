...
When the [care-gaps](OperationDefinition-care-gaps.html) operation is run on the Server, it returns a FHIR Bundle for each patient, conforming to the [DEQM Gaps In Care Bundle Profile]. The `isDocument` parameter determines the shape of that bundle:

- When `isDocument` is true, the bundle is a document bundle containing a Composition that uses the [DEQM Gaps In Care Composition Profile]. The DEQM Gaps In Care Composition references one to many MeasureReport resource; each MeasureReport is for a single measure and conforms to the [DEQM Individual MeasureReport Profile]. Each section also contains a DetectedIssue resource defined using the [DEQM Gaps In Care DetectedIssue Profile], indicating the gap status of that measure via the [DEQM Gap Status Extension], a modifier extension.
- When `isDocument` is false, the bundle is a collection bundle containing the MeasureReport resources and, optionally, the gaps-in-care-specific resources. A Client cannot rely on a Composition or a DetectedIssue being present in this form.

In either form, the MeasureReport resources referenced are packaged in the same DEQM Gaps In Care Bundle, along with Patient, Organization, and other resources that were used to calculate this measure. See [Gaps in Care Bundle Structure](gaps-in-care-reporting.html#gaps-in-care-bundle-structure) for the detail of both shapes.
...
As described below in the section "DEQM Operation Bundles Organized by Subject," Bundles utilized in DEQM operations are organized by subject. This reduces the potential for proliferation of duplicate data because resources common to the subject, such as Encounters, would not be repeated across Bundles. As described in ([Resource URL & Uniqueness rules in a bundle](https://hl7.org/fhir/R4/bundle.html#bundle-unique)), a given version of a resource **SHALL** occur only once in a Bundle, and for DEQM that requires consideration of the data of interest and evaluated resources within the Bundle.
